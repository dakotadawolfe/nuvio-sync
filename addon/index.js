...
addon.post("/api/emby/authenticate", async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({ error: 'Emby server URL, username, and password are required.' });
    }

    const { authenticateEmby } = require('./lib/embyStreams');
    const session = await authenticateEmby(serverUrl, username, password);
    ...
    consola.warn(`[Emby] Authentication failed: ${error?.message || error}`);
    res.status(isAuthError ? status : 400).json({
      error: isAuthError
        ? 'Emby rejected those credentials.'
        : 'Could not connect to Emby. Check the server URL and credentials.',
    });
  }
});
...
addon.get("/manifest.json", function (req, res) {
  const host = process.env.HOST_NAME.startsWith('http')
    ...
    const basicManifest = {
        ...
        name: "AIO Addon",
        description: "AIO Addon provides curated Stremio metadata, Trakt recommendations, streaming-provider catalogs, Emby direct streams, RPDB posters, and OpenSubtitles v3 support.",
        logo: `${host}/logo.png`,
        types: ["movie", "series"],
        catalogs: [],
        resources: [],
...
addon.get("/stremio/:userUUID/stream/:type/:id.json", async function (req, res) {
  ...
  try {
    ...
    if (config.showRateMeButton && id) {
      ...
      const streamUrl = `${host}/stremio/${userUUID}/rating?id=${encodeURIComponent(cleanId)}&type=${type}`;
      streams.push({ externalUrl: streamUrl, name: `Rate Me` });
    }

    return respond(req, res, { streams }, { cacheMaxAge: 0 });
  } catch (error) {
    consola.error(`[Stream Route] Error - userUUID: ${userUUID}, type: ${type}, id: ${id}, error: ${error.message}`, {
      stack: error.stack
    });
    return respond(req, res, { streams: [] }, { cacheMaxAge: 0 });
  }
...
// --- Legacy stream route for rating page ---
addon.get("/stremio/:userUUID/rate-stream/:type/:id.json", async function (req, res) {
  const { userUUID, type, id } = req.params;
  const config = await loadConfigFromDatabase(userUUID);
  if (!config) {
    consola.debug(`[Stream Route] No config found for user: ${userUUID}`);
...
// --- Subtitle Route (for watch tracking and built-in OpenSubtitles v3) ---
...
addon.get("/stremio/:userUUID/subtitles/:type/:id{/:extra}.json", async function (req, res) {
  ...
  try {
    // Load config from database
    const loadedConfig = await loadConfigFromDatabase(userUUID);
    ...
    if (shouldTrackMdblist || shouldTrackAnilist || shouldTrackSimkl || shouldTrackTrakt) {
      const { handleSubtitleRequest } = require('./lib/subtitleHandler');

      // Keep watch tracking as a fire-and-forget side effect; subtitle results come from OpenSubtitles.
    ...
    const subtitles = await fetchOpenSubtitles(type, id, extra, config, queryString);
