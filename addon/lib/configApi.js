...
class ConfigApi {
  ...
  stripLegacyEmbyCredentials(config) {
    if (!config?.apiKeys || typeof config.apiKeys !== 'object') {
      return { cleaned: false };
    }

    let cleaned = false;
    for (const key of ['embyUsername', 'embyPassword']) {
      if (Object.prototype.hasOwnProperty.call(config.apiKeys, key)) {
  ...
  sanitizeApiKeysForResponse(apiKeys = {}) {
    const sanitizedApiKeys = {
      ...apiKeys,
      customDescriptionBlurb: undefined
    };
    delete sanitizedApiKeys.embyUsername;
    delete sanitizedApiKeys.embyPassword;
    return sanitizedApiKeys;
  }

  // Validate required API keys
  validateRequiredKeys(config) {
    const requiredKeys = ['tmdb', 'tvdb', 'rpdb', 'traktTokenId', 'emby'];
    ...
    const missingKeys = requiredKeys.filter(key => {
      if (key === 'tmdb') {
        ...
        const hasBuiltInKey = !!(process.env.TMDB_API_KEY || process.env.TMDB_API || process.env.BUILT_IN_TMDB_API_KEY);
        return !hasUserKey && !hasBuiltInKey;
      }
      if (key === 'tvdb') {
        const hasUserKey = config.apiKeys?.tvdb?.trim();
        const hasBuiltInKey = !!(process.env.TVDB_API_KEY || process.env.BUILT_IN_TVDB_API_KEY);
        return !hasUserKey && !hasBuiltInKey;
      }
      if (key === 'emby') {
        return !(
    ...
    if (missingKeys.length > 0) {
      return {
        valid: false,
        missingKeys,
        message: `Missing required integrations: ${missingKeys.join(', ')}`
      };
    }
  ...
  async saveConfig(req, res) {
    ...
    try {
      ...
      this.stripLegacyEmbyCredentials(config);
  ...
  // Load configuration by UUID (requires password)
    ...
    try {
      ...
      const sanitizedConfig = {
        ...config,
        apiKeys: this.sanitizeApiKeysForResponse(config.apiKeys)
      };
