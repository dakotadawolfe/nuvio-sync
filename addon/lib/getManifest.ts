...
const { hasEmbyCredentials }: any = require('./embyStreams');
...
async function getManifest(config: any, opts: { tag?: string } = {}): Promise<any> {
  ...
  const baseName = config.addonName || (nameSuffix ? `AIO Addon ${nameSuffix}` : "AIO Addon");
  ...
  if(config.showRateMeButton || hasEmbyCredentials(config)) {
