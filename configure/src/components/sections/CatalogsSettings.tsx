...
import { allCatalogDefinitions, defaultStreamingServiceIds, getDefaultCatalogDefinitions, getDefaultCatalogState } from '@/data/catalogs';
...
function CatalogsSettingsContent({
  ...
  const handleReloadCatalogs = () => {
    setConfig(prev => {
      const defaultCatalogs = getDefaultCatalogDefinitions()
        .map(c => {
          const defaultState = getDefaultCatalogState(c);
          return {
            id: c.id,
      ...
      return {
        ...prev,
        streaming: Array.from(new Set([...(prev.streaming || []), ...defaultStreamingServiceIds])),
