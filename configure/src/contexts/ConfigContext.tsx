...
import { allCatalogDefinitions, allSearchProviders, defaultStreamingServiceIds, getDefaultCatalogDefinitions, getDefaultCatalogState } from "@/data/catalogs";
...
function sanitizeApiKeys(apiKeys?: Record<string, any>): Record<string, any> {
  const sanitized = { ...(apiKeys || {}) };
  delete sanitized.embyUsername;
  delete sanitized.embyPassword;
  return sanitized;
}
...
function initializeConfigFromSources(): AppConfig | null {
  ...
  if (loadedConfig) {
    loadedConfig.apiKeys = sanitizeApiKeys(loadedConfig.apiKeys);
...
const initialConfig: AppConfig = {
  ...
  catalogs: getDefaultCatalogDefinitions()
    .map(c => {
      const defaultState = getDefaultCatalogState(c);
      return {
        id: c.id,
        ...
        showInHome: defaultState.showInHome,
  ...
  streaming: defaultStreamingServiceIds,
  customPosterUrlPattern: '',
  customBackgroundUrlPattern: '',
  customLogoUrlPattern: '',
};

const defaultCatalogs = getDefaultCatalogDefinitions().map(c => {
  const defaultState = getDefaultCatalogState(c);
  return {
    id: c.id,
    name: defaultState.name,
  ...
  const [config, setConfig] = useState<AppConfig>(() => {
    if (preloadedConfig) {
      ...
      return {
        ...
        apiKeys: { ...initialConfig.apiKeys, ...sanitizeApiKeys(preloadedConfig.apiKeys) },
      ...
      try {
        ...
        setConfig(currentConfig => ({
          ...
          apiKeys: {
            ...
            ...sanitizeApiKeys(preloadedConfig?.apiKeys), // Priority 1: User's saved keys (from URL or localStorage)
