...
export const baseCatalogs: CatalogDefinition[] = [
  { id: 'tmdb.trending', name: 'Trending (Movies)', type: 'movie', source: 'tmdb', isEnabledByDefault: true, showOnHomeByDefault: true },
  { id: 'tmdb.trending', name: 'Trending (Series)', type: 'series', source: 'tmdb', isEnabledByDefault: true, showOnHomeByDefault: true },
  { id: 'tmdb.top', name: 'Popular (Movies)', type: 'movie', source: 'tmdb', isEnabledByDefault: true, showOnHomeByDefault: true },
  { id: 'tmdb.top', name: 'Popular (Series)', type: 'series', source: 'tmdb', isEnabledByDefault: true, showOnHomeByDefault: true },
...
export const authCatalogs: CatalogDefinition[] = [
  { id: 'trakt.recommendations.movies', name: 'Recommended (Movies)', type: 'movie', source: 'trakt', isEnabledByDefault: true, showOnHomeByDefault: true },
  { id: 'trakt.recommendations.shows', name: 'Recommended (TV)', type: 'series', source: 'trakt', isEnabledByDefault: true, showOnHomeByDefault: true },
];
...
const defaultCatalogKeys = [
  'trakt.recommendations.movies:movie',
  'trakt.recommendations.shows:series',
  'tmdb.trending:movie',
  'tmdb.trending:series',
  'tmdb.top:movie',
  'tmdb.top:series',
  'streaming.nfx:movie',
  'streaming.nfx:series',
  'streaming.hbm:movie',
  'streaming.hbm:series',
  'streaming.dnp:movie',
  'streaming.dnp:series',
  'streaming.amp:movie',
  'streaming.amp:series',
  'streaming.atp:movie',
  'streaming.atp:series',
  'streaming.pmp:movie',
  'streaming.pmp:series',
  'streaming.pcp:movie',
  'streaming.pcp:series',
  'streaming.hlu:movie',
  'streaming.hlu:series',
  'streaming.cru:series',
  'streaming.cru:movie',
] as const;

const defaultCatalogOrder = new Map(defaultCatalogKeys.map((key, index) => [key, index]));

export const defaultStreamingServiceIds = Array.from(
  new Set(
    defaultCatalogKeys
      .filter(key => key.startsWith('streaming.'))
      .map(key => key.split(':')[0].replace('streaming.', ''))
  )
);

function getCatalogKey(catalog: CatalogDefinition) {
  return `${catalog.id}:${catalog.type}`;
}

export function getDefaultCatalogState(catalog: CatalogDefinition) {
  const enabled = defaultCatalogOrder.has(getCatalogKey(catalog));

  return {
    name: catalog.name,
    enabled,
    showInHome: enabled && (catalog.showOnHomeByDefault || false),
  };
}

export function getDefaultCatalogDefinitions() {
  return allCatalogDefinitions
    .filter(catalog => defaultCatalogOrder.has(getCatalogKey(catalog)))
    .sort((a, b) => {
      const aOrder = defaultCatalogOrder.get(getCatalogKey(a)) ?? Number.MAX_SAFE_INTEGER;
      const bOrder = defaultCatalogOrder.get(getCatalogKey(b)) ?? Number.MAX_SAFE_INTEGER;
